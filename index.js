...
<title>LOOKLI – מנוע חיפוש חכם לאופנה צנועה | אלפי מוצרים במקום אחד</title>
  <style>
    ...
    *{box-sizing:border-box;font-family:'Segoe UI',system-ui,-apple-system,sans-serif}
    ...
    .store-opt::after{content:attr(data-store);position:absolute;bottom:calc(100% + 5px);left:50%;transform:translateX(-50%);background:rgba(0,0,0,.8);color:#fff;font-size:11px;font-weight:700;padding:4px 8px;border-radius:6px;white-space:nowrap;opacity:0;pointer-events:none;transition:opacity .15s;z-index:999}
    ...
    .card-logo::after{content:attr(data-store);position:absolute;top:calc(100% + 5px);right:0;background:rgba(0,0,0,.8);color:#fff;font-size:11px;font-weight:700;padding:4px 9px;border-radius:6px;white-space:nowrap;opacity:0;pointer-events:none;transition:opacity .15s;z-index:99}
  ...
  <div class="wrap">
    ...
    <aside class="filters" id="filtersPanel">
      ...
      <div style="padding:14px 20px">
        <button id="netfreeBtn" onclick="toggleNetfree()" style="width:100%;padding:12px;margin-top:10px;border:2px solid var(--border);border-radius:10px;background:#fff;cursor:pointer;font-weight:700;font-size:13px;color:var(--muted);transition:all .2s;display:flex;align-items:center;justify-content:center;gap:8px">
          <span>🌐</span> <span id="netfreeBtnText">הצג רק תמונות פתוחות</span>
        </button>
...
let activeNetfree=false;

function toggleNetfree(){
  activeNetfree=!activeNetfree;
  const btn=document.getElementById('netfreeBtn');
  const txt=document.getElementById('netfreeBtnText');
  if(activeNetfree){
    btn.style.borderColor='var(--accent)';
    btn.style.color='var(--accent)';
    btn.style.background='rgba(224,161,192,.08)';
    txt.textContent='מציג רק תמונות פתוחות ✓';
  } else {
    btn.style.borderColor='var(--border)';
    btn.style.color='var(--muted)';
    btn.style.background='#fff';
    txt.textContent='הצג רק תמונות פתוחות';
  }
  runRegularSearch();
}
...
const storeLogos={
  ...
  'MYME':'https://myme.co.il/wp-content/uploads/2023/07/black-logo.png'
};
...
function renderProducts(products,append=false){
  ...
  for(const p of products){
    ...
    let carouselHTML='';
    if(images.length>1){
      const slides=thumbImages.slice(0,6).map((img,si)=>`<div class="carousel-slide"><img src="${escapeHtml(img)}" alt="${escapeHtml(p.title||'')} - תמונה ${si+1}" loading="lazy" ${si===0?'onload="checkNetfreeBlock(this)" onerror="var _c=this.closest(\'.card\');if(_c)_c.style.display=\'none\'"':''}>\u200B</div>`).join('');
      ...
      carouselHTML=`<img src="${escapeHtml(thumbImages[0])}" alt="${escapeHtml((p.title||'')+' - '+(p.store||'')+' | LOOKLI')}" width="300" height="400" style="width:100%;height:100%;object-fit:cover" ${cardIndex<4?'fetchpriority="high"':'loading="lazy"'} onload="checkNetfreeBlock(this)" onerror="const _c=this.closest&&this.closest('.card');if(_c)_c.style.display='none'">`;
    ...
    const logoHTML = logoSrc ? `<div class="card-logo" data-store="${store}" ${logoBgClass}><img src="${logoSrc}" alt="${store}"${localLogo ? ` onerror="this.src='${fallbackLogo}'"` : ''}></div>` : '';
...
async function initFilters(){
  ...
  (data.stores||[]).forEach(s=>{const fallback=storeLogos[s];const bgStyle=darkBgStores.has(s)?'style="background:#1a1a2e"':'';const localOnly=new Set(['CHEN','ORDMAN']);const src=localOnly.has(s)?`/${s.toLowerCase()}-logo.png`:fallback;storeDiv.innerHTML+=`<div class="store-opt" data-store="${s}" onclick="selectStore('${s}')" ${bgStyle}>${src?`<img src="${src}" alt="${s}"${localOnly.has(s)?` onerror="this.src='${fallback}'"`:''}`:`<span>${s.substring(0,3)}</span>`}</div>`});
...
function resetAll(){
  ...
  resetAllFiltersQuietly();
  // איפוס נטפרי
  if(activeNetfree){activeNetfree=false;const btn=document.getElementById('netfreeBtn');btn.style.borderColor='var(--border)';btn.style.color='var(--muted)';btn.style.background='#fff';document.getElementById('netfreeBtnText').textContent='הצג רק תמונות פתוחות';}
  document.getElementById("aiAnalysis").classList.remove("active");
  runRegularSearch();
}
    ...
    try {
      ...
      el.innerHTML = recent.map(p => `
        <div onclick="closeAuthModal();openProduct(${p.id})" style="display:flex;align-items:center;gap:10px;padding:8px;border-radius:10px;cursor:pointer;border:1px solid #f3f4f6;transition:background .15s" onmouseenter="this.style.background='#fdf2f8'" onmouseleave="this.style.background=''">
          <img src="${escapeHtml(p.image||'')}" style="width:44px;height:44px;object-fit:cover;border-radius:8px;flex-shrink:0" onerror="this.style.display='none'">
    ...
    try {
      ...
      el.innerHTML = recent.map(p => `
        <div onclick="closeProfilePanel();openProduct(${p.id})" style="display:flex;align-items:center;gap:10px;padding:8px;border-radius:10px;cursor:pointer;border:1px solid #f3f4f6;transition:background .15s" onmouseenter="this.style.background='#fdf2f8'" onmouseleave="this.style.background=''">
          <img src="${escapeHtml(p.image||'')}" style="width:48px;height:48px;object-fit:cover;border-radius:8px;flex-shrink:0" onerror="this.style.display='none'">
          <div style="flex:1;min-width:0">
            <div style="font-size:13px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escapeHtml(p.title||'')}</div>
