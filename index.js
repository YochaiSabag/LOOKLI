...
<head>
  ...
  <meta name="google-site-verification" content="i7jqYb-ITZPAK9yGgZ0se9f2pGAm7A5sJHDfKDi6ryY"/>
  ...
  <link rel="dns-prefetch" href="https://aviyahyosef.com"/>
  <link rel="dns-prefetch" href="https://chemise.co.il"/>
...
<title>LOOKLI – מנוע חיפוש חכם לאופנה צנועה | אלפי מוצרים במקום אחד</title>
  <style>
    ...
    *{box-sizing:border-box;font-family:'Segoe UI',system-ui,-apple-system,sans-serif}
    ...
    /* ===== SIDEBAR ===== */
    .filters{width:var(--sidebar);background:#fff;border-left:1px solid var(--border);padding:0;flex-shrink:0;overflow-y:auto;overflow-x:visible;height:calc(100vh - 136px);position:sticky;top:136px}
    ...
    .filter-opt{padding:10px 16px;border:2px solid var(--border);border-radius:8px;background:#fff;cursor:pointer;font-weight:700;font-size:14px;transition:all .15s;color:var(--muted);line-height:1.2;white-space:nowrap;margin:0}
    ...
    /* Store logos */
    .store-opts{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;overflow:visible}
    .store-opt{width:100%;aspect-ratio:1;border:2px solid var(--border);border-radius:10px;background:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .15s;overflow:visible;flex-shrink:0;position:relative}
    ...
    .store-opt img{width:calc(100% - 8px);height:calc(100% - 8px);object-fit:contain;border-radius:7px}
    .store-opt[data-store="MYME"] img{width:100%;height:100%;padding:2px}
    .store-opt span{font-size:12px;font-weight:800}
    .store-opt::after{content:attr(data-tooltip);position:absolute;bottom:calc(100% + 5px);left:50%;transform:translateX(-50%);background:rgba(0,0,0,.8);color:#fff;font-size:11px;font-weight:700;padding:4px 8px;border-radius:6px;white-space:nowrap;opacity:0;pointer-events:none;transition:opacity .15s;z-index:999}
    .store-opt:hover::after{opacity:1}
    ...
    .sort-wrap{position:relative;display:inline-block;user-select:none}
    .sort-btn{
    ...
    .sort-btn.open{border-color:var(--accent);color:var(--accent);box-shadow:0 2px 10px rgba(224,161,192,.25)}
    .sort-arrow{
    ...
    .sort-item:hover{background:#fdf0f7;color:var(--accent)}
    .sort-item.active{background:linear-gradient(135deg,#fce8f3,#f8e0ef);color:var(--accent);font-weight:700}
    ...
    .card-logo::after{content:attr(data-tooltip);position:absolute;top:calc(100% + 5px);right:0;background:rgba(0,0,0,.8);color:#fff;font-size:11px;font-weight:700;padding:4px 9px;border-radius:6px;white-space:nowrap;opacity:0;pointer-events:none;transition:opacity .15s;z-index:99}
    .card-logo:hover::after{opacity:1}
    ...
    /* ===== MODAL ===== */
    /* הbackdrop עצמו לא גולל — הגלילה בתוך הwrapper הפנימי */
    .modal-backdrop{position:fixed;inset:0;background:rgba(0,0,0,.6);display:none;z-index:200;padding:16px;overflow:hidden;backdrop-filter:blur(4px);align-items:flex-start;justify-content:center}
    .modal-backdrop.open{display:flex}
    #modalWrapper{max-height:calc(100vh - 32px);overflow-y:auto;overflow-x:hidden;border-radius:18px;scrollbar-width:thin;scrollbar-color:var(--accent) transparent}
    #modalWrapper::-webkit-scrollbar{width:5px}
    #modalWrapper::-webkit-scrollbar-thumb{background:var(--accent);border-radius:3px}
    .modal{width:min(1000px,100%);background:#fff;border-radius:18px;overflow:visible;display:flex;flex-direction:column;box-shadow:0 25px 80px rgba(0,0,0,.25)}
    .modal-head{padding:8px 18px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid var(--border);flex-shrink:0}
    .modal-title{font-weight:800;font-size:15px;max-width:85%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    .close-btn{width:32px;height:32px;border-radius:8px;border:2px solid var(--border);background:#fff;cursor:pointer;font-size:15px;transition:all .15s}
    .close-btn:hover{border-color:var(--accent);color:var(--accent)}
    .modal-body{display:grid;grid-template-columns:1fr 1fr;gap:14px;padding:12px 16px}
    @media(max-width:800px){.modal-body{grid-template-columns:1fr}}
    
    /* Modal Gallery */
    .gallery{border:1px solid var(--border);border-radius:14px;overflow:hidden;background:#fff}
    .gallery-carousel{position:relative;width:100%;overflow:hidden;background:#fff}
    ...
    .gallery-carousel-slide img{width:100%;height:auto;object-fit:contain;max-height:68vh}
    ...
    @media(max-width:640px){#similarGrid{grid-template-columns:repeat(2,1fr)}}
...
<!-- Google tag (gtag.js) -->
<script async src="https://www.googletagmanager.com/gtag/js?id=G-6BG63T2LEF"></script>
<script src="https://accounts.google.com/gsi/client" async defer></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());
  gtag('config', 'G-6BG63T2LEF', {
...
<body>
    ...
    <div class="search-section">
      <div class="search-inner">
        <div class="searchbar">
          ...
          <button class="btn" onclick="document.getElementById('imgSearchInput').click()" title="חיפוש לפי תמונה" style="background:var(--accent2,#7c6fcf)">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="12" cy="12" r="4"/><line x1="6.5" y1="6.5" x2="6.5" y2="6.5"/></svg>
          </button>
          <input id="imgSearchInput" type="file" accept="image/*" style="display:none" onchange="searchByImage(event)"/>
        </div>
      </div>
    </div>
  </header>
  ...
  <div class="wrap">
    ...
    <aside class="filters" id="filtersPanel">
      <button class="filter-close-btn" onclick="toggleMobileFilters()" aria-label="סגור פילטרים">✕</button>
      ...
      <div class="toolbar">
        ...
        <div class="sort-wrap" id="sortWrap">
          <button class="sort-btn" id="sortBtn" onclick="toggleSortMenu()">
            <span id="sortLabel">חדש → ישן</span>
            <span class="sort-arrow"><svg viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"/></svg></span>
          </button>
          <div class="sort-menu" id="sortMenu">
            <div class="sort-item active" data-val="new" onclick="selectSort('new','חדש → ישן',this)">חדש → ישן</div>
            <div class="sort-item" data-val="popular" onclick="selectSort('popular','פופולרי',this)">פופולרי</div>
            <div class="sort-item" data-val="price_asc" onclick="selectSort('price_asc','מחיר ↑',this)">מחיר ↑</div>
            <div class="sort-item" data-val="price_desc" onclick="selectSort('price_desc','מחיר ↓',this)">מחיר ↓</div>
          </div>
        </div>
      </div>
  ...
  <div class="modal-backdrop" id="modalBackdrop" onclick="closeModal(event)">
    <div id="modalWrapper" onclick="event.stopPropagation()" style="width:min(1000px,100%);background:#fff;border-radius:18px;box-shadow:0 25px 80px rgba(0,0,0,.25);display:flex;flex-direction:column">
    <div class="modal" style="box-shadow:none;border-radius:0;width:100%;flex-shrink:0;max-height:none">
      <div class="modal-head">
        <div class="modal-title" id="modalTitle">פרטי מוצר</div>
        <button class="close-btn" onclick="closeModal()" aria-label="סגור">✕</button>
      </div>
      <div class="modal-body">
        ...
        <div class="details">
          ...
          <div class="modal-colors-section">
            ...
            <div id="modalColorSizes" style="display:none;margin-top:8px;font-size:13px;color:#374151">
              <span id="modalColorSizesLabel" style="color:#9ca3af;font-size:11px;margin-bottom:4px;display:block"></span>
              <div id="modalColorSizesTags" style="display:flex;flex-wrap:wrap;gap:6px;margin-top:4px"></div>
            </div>
          </div>
          <div>
          ...
          <div class="buy-big">
            <div style="display:flex;gap:8px;align-items:stretch">
              <a id="modalBuy" href="#" target="_blank" onclick="trackOutClick(this)" style="flex:1"><svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:20px;height:20px;vertical-align:middle;margin-left:8px"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>לרכישה באתר</a>
              <button onclick="shareProduct()" title="שתפי בוואצאפ" style="padding:0 16px;border-radius:12px;border:none;background:#25D366;color:#fff;cursor:pointer;font-size:20px;transition:opacity .2s" onmouseenter="this.style.opacity='.85'" onmouseleave="this.style.opacity='1'">
                <svg viewBox="0 0 24 24" fill="white" style="width:20px;height:20px;display:block"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
              </button>
            </div>
          </div>
        </div>
    ...
    <!-- מוצרים דומים — מחובר ישירות מתחת, בלי רווח -->
    <div id="similarSection" onclick="event.stopPropagation()" style="display:none;border-top:1px solid #f3f4f6;padding:20px 24px 24px">
      <div style="font-size:12px;font-weight:700;color:#9ca3af;letter-spacing:.5px;text-transform:uppercase;margin-bottom:14px">מוצרים דומים</div>
      <div id="similarGrid" style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px"></div>
    </div>
...
const activeNetfree=false;
function handleImgError(img){
  console.warn('[IMG FAIL]', img.src);
  img.style.display='none';
  var w=img.closest('.img-wrap');
  if(w&&!w.querySelector('.img-err')){
    var d=document.createElement('div');
    d.className='img-err';
    d.style.cssText='position:absolute;inset:0;display:flex;align-items:center;justify-content:center;color:#d9b8d0;font-size:36px';
    d.textContent='👗';
    w.appendChild(d);
  }
}
...
let currentPage=1,itemsPerPage=isMobile?8:20,allProducts=[],isLoading=false;
...
const storeLogos={
  ...
  'CHEMISE':'/chemise.png',
  'CHEN':'/chen-logo.png',
  'AVIVIT':'https://avivit-weizman.co.il//wp-content/uploads/2025/06/%D7%A2%D7%99%D7%A6%D7%95%D7%91-%D7%9C%D7%9C%D7%90-%D7%A9%D7%9D-3.png',
  'RARE':'https://rare.co.il/490081_4076.webp',
  'ORDMAN':'https://www.ordman.co.il/wp-content/uploads/2025/09/%D7%98%D7%A7%D7%A1%D7%98-%D7%94%D7%A4%D7%A1%D7%A7%D7%94-%D7%A9%D7%9C%D7%9A-50-e1757934226395.png',
  'ST-FASHION':'/st-fashion.png',
  'SALINA':'https://salinafashion.com/wp-content/uploads/2023/07/LOGO-SALINA.png',
  'MYME':'/myme.png',
  'SHEBELLO':'/shebello.png',
  'MODA':'/moda.png',
  'EUROPISRAEL':'/europisrael.png',
  'LEAA':'/leaa.png'
};

// פונקציה שמחזירה לוגו — קודם בודקת קובץ מקומי בפובליק, אחר כך URL חיצוני
function getStoreLogo(store) {
  // אם יש קובץ מקומי בשם החנות — השתמש בו (png/jpg/svg/webp)
  const localExts = ['png','jpg','svg','webp'];
  for (const ext of localExts) {
    const path = `/${store.toLowerCase()}-logo.${ext}`;
    // נחזיר את הנתיב המקומי אם קיים — הדפדפן ינסה וישתמש ב-onerror לfallback
  }
  return storeLogos[store] || null;
}

// חנויות שהלוגו שלהן לבן וצריכות רקע כהה
const darkBgStores = new Set([]);
...
// מצמצם URL של תמונה לגודל כרטיסייה (~400px)
function thumbUrl(url) {
  if (!url) return url;
  // חנויות עם hotlink protection — נתב דרך proxy
  // תמונות שנשמרו ב-DB על ידי הסקרייפר — כבר בפורמט /ic?u=...
  if (url.startsWith('/ic?') || url.startsWith('/img?')) return url;
  // תמונות chemise ישנות שעדיין ב-DB — נתב דרך /img proxy
  if (url.includes('chemise.co.il')) {
    return '/img?url=' + encodeURIComponent(url.replace(/-\d+x\d+(\.\w+)$/, '$1'));
  }
  // WooCommerce — החלף גודל קיים ל-400x600
  if (url.includes('wp-content/uploads')) {
    // אם יש כבר גודל כמו -1708x2560 — החלף ל-400x600
    return url.replace(/-\d+x\d+(\.\w+)$/, '-400x600$1');
  }
  // Wix — הוסף פרמטרי resize
  if (url.includes('wixstatic.com')) {
    return url.replace(/\/v1\/fill\/[^/]+\//, '/v1/fill/w_400,h_600,al_c,q_80,usm_0.66_1.00_0.01,enc_avif/');
  }
  return url;
}
...
async function searchByImage(event){
  ...
  try{
    ...
    // שלב ב: ניתוח תמונה (Claude)
    const analyzeResp=await fetch('/api/analyze-image',{
      method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({imageBase64:base64,mimeType:'image/jpeg'})
    });
    const analyzeText=await analyzeResp.text();
    let analyzeData;
    try{analyzeData=JSON.parse(analyzeText);}catch{alert('שגיאה בניתוח תמונה');runRegularSearch();return;}
    if(!analyzeResp.ok){alert(analyzeData.error||'שגיאה בניתוח');runRegularSearch();return;}
    const analysis=analyzeData.analysis;

    // שלב ג: חיפוש ב-DB לפי הפרמטרים (מהיר)
    const searchResp=await fetch('/api/search-by-params',{
      method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify(analysis)
    });
    const searchText=await searchResp.text();
    let data;
    try{data=JSON.parse(searchText);}catch{alert('שגיאה בחיפוש');runRegularSearch();return;}
    if(!searchResp.ok){alert(data.error||'שגיאה');runRegularSearch();return;}

    lastSearchKeywords=analysis.keywords||[];
    resetAllFiltersQuietly();
    if(analysis.color)activeColors=[analysis.color];
    if(analysis.category)activeCategories=[analysis.category];
    if(analysis.size)activeSizes=[analysis.size];
    if(analysis.style)activeStyles=[analysis.style];
    if(analysis.fit)activeFits=[analysis.fit];
    if(analysis.fabric)activeFabrics=[analysis.fabric];
    if(analysis.pattern)activePattern=analysis.pattern;
    ...
    document.getElementById('metaToolbar').textContent=`${data.count} תוצאות (חיפוש תמונה)`;
  }catch(e){console.error(e);alert('שגיאה בחיפוש תמונה');runRegularSearch();}
}
...
async function runRegularSearch(){
  ...
  if(window._sortVal&&window._sortVal!=='new')params.set("sort",window._sortVal);
  ...
  if (window._productCache[cacheKey]) {
    data = window._productCache[cacheKey];
  } else {
    const res = await fetch("/api/products?" + params);
    data = await res.json();
    window._productCache[cacheKey] = data;
    // ניקוי cache אם גדל מדי (מעל 10 entries)
    const keys = Object.keys(window._productCache);
    if (keys.length > 10) delete window._productCache[keys[0]];
  }
  
  allProducts = (window._sortVal === 'new' || !window._sortVal) ? interleaveByStore(data) : data;
...
function renderProducts(products,append=false){
  ...
  let cardIndex = append ? grid.children.length : 0;
  
  for(const p of products){
    const store=p.store||"MEKIMI",logo=storeLogos[store]||'';
    const images=(p.images?.length)?p.images:(p.image_url?[p.image_url]:[]);
    const thumbImages = images.map(thumbUrl); // גודל מצומצם לכרטיסיות
    const colors=[...new Set(p.colors||(p.color?[p.color]:[]))];
    ...
    let carouselHTML='';
    if(images.length>1){
      const slides=thumbImages.slice(0,6).map((img,si)=>`<div class="carousel-slide"><img src="${escapeHtml(img)}" alt="${escapeHtml(p.title||'')} - תמונה ${si+1}" loading="lazy" ${si===0?'onload="checkNetfreeBlock(this)" onerror="handleImgError(this)"':''}>\u200B</div>`).join('');
      const dots=thumbImages.slice(0,6).map((_,i)=>`<span class="carousel-dot${i===0?' active':''}" onclick="event.stopPropagation();goToSlide(this,${i})"></span>`).join('');
      carouselHTML=`<div class="carousel" data-index="0"><div class="carousel-inner">${slides}</div><div class="carousel-dots">${dots}</div><button class="carousel-nav carousel-prev" onclick="event.stopPropagation();carouselNav(this,-1)" aria-label="הקודם">‹</button><button class="carousel-nav carousel-next" onclick="event.stopPropagation();carouselNav(this,1)" aria-label="הבא">›</button></div>`;
    }else if(images.length===1){
      carouselHTML=`<img src="${escapeHtml(thumbImages[0])}" alt="${escapeHtml((p.title||'')+' - '+(p.store||'')+' | LOOKLI')}" width="300" height="400" style="width:100%;height:100%;object-fit:cover" ${cardIndex<4?'fetchpriority="high"':'loading="lazy"'} onload="checkNetfreeBlock(this)" onerror="handleImgError(this)">`;
    ...
    if(colors.length>0){
      ...
      colors.slice(0,3).forEach(c=>{const hex=getColorHex(c);const isDark=c==='שחור'||['#000000','#0a0a0a','#111111'].includes(hex.toLowerCase());colorDotsHTML+=`<span class="card-color" style="background:${hex};${isDark?'box-shadow:0 0 0 1.5px #aaa inset;':''}" title="${escapeHtml(c)}"></span>`});
    ...
    const localLogoStores = new Set(['CHEN', 'ORDMAN']); // רק חנויות עם לוגו מקומי ידוע
    const localLogo = localLogoStores.has(store) ? `/${store.toLowerCase()}-logo.png` : null;
    const fallbackLogo = storeLogos[store] || '';
    const logoSrc = localLogo || fallbackLogo;
    const storeDisplayNames={'LEAA':'Ladies'};
    const logoHTML = logoSrc ? `<div class="card-logo" data-store="${store}" data-tooltip="${storeDisplayNames[store]||store}" ${logoBgClass}><img src="${logoSrc}" alt="${store}"${localLogo ? ` onerror="this.src='${fallbackLogo}'"` : ''}></div>` : '';
    ...
    grid.innerHTML+=`<div class="card" onclick="openProduct(${p.id})"><div class="img-wrap">${saleBadgeHTML}${carouselHTML}${logoHTML}${colorDotsHTML}${saveBtn}</div><div class="card-body"><div class="title">${escapeHtml(p.title||'')}</div><div class="subrow">${priceHTML}<div class="store">${escapeHtml(store)}</div></div><div class="card-footer"><div class="sizes">${sizes.join(', ')||'-'}</div>${shippingBadgeHTML}</div></div></div>`;
    cardIndex++;
  }
}
...
async function initFilters(){
  ...
  // מזג hex צבעים מה-DB לתוך colorData (ניצחון DB על קשיח)
  if(data.colorHex) Object.assign(colorData, data.colorHex);
  
  const storeDiv=document.getElementById("storeLogos");
  storeDiv.innerHTML=`<div class="store-opt active" data-store="הכל" onclick="selectStore('')"><span>הכל</span></div>`;
  const storeDisplayNames={'LEAA':'Ladies'};
  (data.stores||[]).forEach(s=>{const fallback=storeLogos[s];const bgStyle=darkBgStores.has(s)?'style="background:#1a1a2e"':'';const localOnly=new Set(['CHEN','ORDMAN']);const src=localOnly.has(s)?`/${s.toLowerCase()}-logo.png`:fallback;const displayName=storeDisplayNames[s]||s;storeDiv.innerHTML+=`<div class="store-opt" data-store="${s}" data-tooltip="${displayName}" onclick="selectStore('${s}')" ${bgStyle}>${src?`<img src="${src}" alt="${s}"${localOnly.has(s)?` onerror="this.src='${fallback}'"`:''}`:`<span>${s.substring(0,3)}</span>`}</div>`});
...
function shareProduct() {
  ...
  const text = `מצאתי את זה ב-LOOKLI 🛍️\n${title}\n${url}`;
...
function resizeImg(url, width) {
  if (!url) return url;
  try {
    // בנייד — הקטן רזולוציה (עדיין איכותי)
    const targetWidth = isMobile ? Math.min(width, 480) : width;
    // WooCommerce — מחליף -WxH ב-URL
    if (url.match(/\-\d+x\d+\.(jpg|jpeg|png|webp)/i)) {
      return url.replace(/\-\d+x\d+(\.(jpg|jpeg|png|webp))/i, `-${targetWidth}x${Math.round(targetWidth*4/3)}$1`);
    }
    // Wix CDN — תומך בפרמטרים
    if (url.includes('wixstatic.com')) {
      return url.replace(/w_\d+/, `w_${targetWidth}`).replace(/h_\d+/, `h_${Math.round(targetWidth*4/3)}`);
    }
    return url;
  } catch(e) { return url; }
}
...
async function openProduct(id){
  ...
  document.title='LOOKLI – טוען מוצר…';
  // ניקוי מיידי של כל תוכן המוצר הקודם
  ...
  document.getElementById("modalCarouselInner").innerHTML='<div class="gallery-carousel-slide" style="display:flex;align-items:center;justify-content:center;height:300px"><div style="width:40px;height:40px;border:3px solid #e5e7eb;border-top-color:var(--accent);border-radius:50%;animation:spin 0.8s linear infinite"></div></div>';
  ...
  try{
    ...
    document.title=`${p.title||'מוצר'} – LOOKLI`;
    const slug=titleToSlug(p.title);
    if(slug) history.replaceState({productId:id},'',`/product/${slug}`);

    // שמור בהיסטוריית צפייה
    addToRecentlyViewed(p);

    // שלב 1 — נתונים וכפתור רכישה (מיידי)
    ...
    const colors=[...new Set(p.colors||(p.color?[p.color]:[]))];
    const colorSizes=p.color_sizes||{};
    const colorsDiv=document.getElementById("modalColors");
    const colorSizesDiv=document.getElementById("modalColorSizes");
    const colorSizesLabel=document.getElementById("modalColorSizesLabel");
    const colorSizesTags=document.getElementById("modalColorSizesTags");

    const showColorSizes = (color) => {
      const sizes = colorSizes[color];
      if (!sizes || sizes.length === 0) { colorSizesDiv.style.display='none'; return; }
      colorSizesLabel.textContent = `מידות ב${color}:`;
      colorSizesTags.innerHTML = sizes.map(s =>
        `<span style="background:#f3f4f6;border-radius:6px;padding:3px 10px;font-size:12px;font-weight:600">${s}</span>`
      ).join('');
      colorSizesDiv.style.display = 'block';
      // סמן צבע פעיל
      colorsDiv.querySelectorAll('.modal-color').forEach(el => el.style.borderColor = '#ddd');
      const active = colorsDiv.querySelector(`[data-color="${color}"]`);
      if (active) active.style.borderColor = '#c48cb3';
    };

    if(colors.length){
      colors.forEach(c=>{
        const dot = document.createElement('span');
        dot.className = 'modal-color';
        dot.style.background = getColorHex(c);
        dot.title = c;
        dot.setAttribute('data-color', c);
        dot.onclick = () => showColorSizes(c);
        colorsDiv.appendChild(dot);
      });
      // הצג אוטומטית את הצבע הראשון שיש לו מידות
      const firstWithSizes = colors.find(c => colorSizes[c]?.length > 0);
      if (firstWithSizes) showColorSizes(firstWithSizes);
    } else {
      colorsDiv.innerHTML='<span style="color:#999;font-size:12px">לא צוין</span>';
    }
    ...
    if(modalImages.length){
      ...
      mainImg.onload = () => {
        document.getElementById("modalCarouselInner").innerHTML=modalImages.map((img,i)=>`<div class="gallery-carousel-slide"><img src="${escapeHtml(img)}" alt="" loading="${i===0?'eager':'lazy'}"></div>`).join('');
        ...
        // שלב 3 — תמונות משנה (אחרי התמונה הראשית)
        setTimeout(()=>{
    ...
    if (!ldScript) { ldScript = document.createElement('script'); ldScript.id='product-jsonld'; ldScript.type='application/ld+json'; document.head.appendChild(ldScript); }
    ldScript.textContent = JSON.stringify({
      ...
      "offers": {
        "@type": "Offer",
        "url": `https://www.lookli.co.il/product/${titleToSlug(p.title)}`,
        "priceCurrency": "ILS",
        "price": p.price || 0,
    ...
    // מוצרים דומים — lazy load עם IntersectionObserver (נטען רק כשגולל אל הסקשן)
    const similarSection = document.getElementById('similarSection');
    similarSection.style.display = 'none';
    document.getElementById('similarGrid').innerHTML = '';
    // ביטול observer קודם אם קיים
    if (window._similarObserver) window._similarObserver.disconnect();
    // שמור ID לטעינה עתידית
    similarSection.dataset.pendingId = id;
    window._similarObserver = new IntersectionObserver((entries) => {
      if (!entries[0].isIntersecting) return;
      window._similarObserver.disconnect();
      const pendingId = similarSection.dataset.pendingId;
      if (!pendingId) return;
      // הראה spinner
      document.getElementById('similarGrid').innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:20px"><div style="display:inline-block;width:28px;height:28px;border:3px solid #f3f4f6;border-top-color:var(--accent);border-radius:50%;animation:spin 0.8s linear infinite"></div></div>';
      fetch(`/api/similar/${pendingId}`).then(r=>r.json()).then(items=>{
        if(!items?.length){similarSection.style.display='none';return;}
        const grid = document.getElementById('similarGrid');
        grid.innerHTML = items.map(p=>{
          const img = (p.images?.[0]||p.image_url||'');
          const isOnSale = p.original_price && p.original_price > p.price;
          const priceHTML = isOnSale
            ? `<span style="color:var(--accent);font-weight:700">₪${p.price}</span> <s style="color:#9ca3af;font-size:11px">₪${p.original_price}</s>`
            : `<span style="font-weight:600">₪${p.price}</span>`;
          return `<div onclick="openProduct(${p.id})" style="cursor:pointer;border-radius:10px;overflow:hidden;border:1px solid #f3f4f6;transition:box-shadow .15s" onmouseenter="this.style.boxShadow='0 4px 12px rgba(0,0,0,.1)'" onmouseleave="this.style.boxShadow=''">
            <div style="aspect-ratio:3/4;background:#f9fafb;overflow:hidden">
              <img src="${escapeHtml(img)}" alt="${escapeHtml(p.title||'')}" loading="lazy" style="width:100%;height:100%;object-fit:cover">
            </div>
            <div style="padding:7px 8px">
              <div style="font-size:11px;color:#6b7280;margin-bottom:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escapeHtml(p.store||'')}</div>
              <div style="font-size:12px;line-height:1.3;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;margin-bottom:4px">${escapeHtml(p.title||'')}</div>
              <div style="font-size:12px">${priceHTML}</div>
            </div>
          </div>`;
        }).join('');
      }).catch(()=>{similarSection.style.display='none';});
    }, { root: document.getElementById('modalWrapper'), threshold: 0.1 });
    // הצג את הסקשן (ריק) כדי שה-observer יוכל לזהות אותו
    similarSection.style.display = 'block';
    window._similarObserver.observe(similarSection);
...
function closeModal(e){
  ...
  document.title='LOOKLI – מנוע חיפוש לאופנה צנועה';
  if(location.pathname.startsWith('/product/')){history.pushState({},'','/');}
}
// כפתור חזרה בדפדפן
window.addEventListener('popstate',e=>{
  if(e.state?.productId){openProduct(e.state.productId);}
  else{document.getElementById("modalBackdrop").classList.remove("open");document.body.style.overflow='';document.title='LOOKLI – מנוע חיפוש לאופנה צנועה';}
});
// כניסה ישירה ל-/product/slug או /product/ID
(function(){
  ...
  if(m){
    ...
    window.addEventListener('DOMContentLoaded',async()=>{
      // אם מספר — פתח לפי ID
      if(/^\d+$/.test(slugOrId)){
        openProduct(slugOrId);
      } else {
        // חפש לפי slug
        try{
          const p=await(await fetch('/api/product/slug/'+slugOrId)).json();
          if(p.id) openProduct(p.id);
        }catch(e){}
      }
    });
  }
...
(function(){
  ...
  const TYPE_SPEED=60, DELETE_SPEED=30, PAUSE_AFTER=1800, PAUSE_BEFORE=400, PAUSE_ON_BLUR=20000;
  let pi=0, ci=0, deleting=false, timer=null, pausedUntil=0;

  function tick(){
    if(Date.now() < pausedUntil){ timer=setTimeout(tick, pausedUntil-Date.now()); return; }
    if(document.activeElement===input){ timer=setTimeout(tick,500); return; }
    const phrase=phrases[pi];
    if(!deleting){
      ci++;
      input.placeholder=phrase.substring(0,ci);
      if(ci===phrase.length){ deleting=true; timer=setTimeout(tick,PAUSE_AFTER); return; }
      timer=setTimeout(tick,TYPE_SPEED);
    } else {
      ci--;
      input.placeholder=phrase.substring(0,ci);
      if(ci===0){ deleting=false; pi=(pi+1)%phrases.length; timer=setTimeout(tick,PAUSE_BEFORE); return; }
      timer=setTimeout(tick,DELETE_SPEED);
    }
  }

  // עצור 20 שניות כשעוזבים את השדה
  input.addEventListener('blur', ()=>{
    pausedUntil = Date.now() + PAUSE_ON_BLUR;
  });

  // טען מה-API, fallback לברירת מחדל
  fetch('/api/search-phrases').then(r=>r.json()).then(d=>{
    if(d.phrases && d.phrases.length) phrases=d.phrases;
  }).catch(()=>{}).finally(()=>{
...
function renderHeroStep(){
  ...
  if(heroStep==='category'){
    ...
    cubesDiv.innerHTML=colors.map(function(c){var is=sel.indexOf(c)!==-1;return '<div class="hero-color-cube'+(is?' selected':'')+'" data-color="'+escapeHtml(c)+'" onclick="heroToggle(\'color\',this.dataset.color,this)"><div class="hero-color-circle" style="background:'+getColorHex(c)+'"></div><span class="hero-cube-label">'+escapeHtml(c)+'</span></div>'}).join('');
    ...
    var validSizeRe=/^(XS|S|M|L|XL|XXL|XXXL|2XL|3XL|4XL|ONE SIZE|OS|FREE SIZE)$/i;
    var filteredDb=dbSizes.filter(function(s){return validSizeRe.test((s||'').trim())});
    // נרמול כפילויות
    var sizeNorm={'3XL':'XXXL','2XL':'XXL'};
    filteredDb=filteredDb.map(function(s){return sizeNorm[s.toUpperCase()]||s;});
    filteredDb=filteredDb.filter(function(s,i){return filteredDb.indexOf(s)===i;});
    var sizes=filteredDb.length>0?filteredDb:popS;
    if(filteredDb.length>0){var o3=[];popS.forEach(function(s){if(sizes.indexOf(s)!==-1)o3.push(s)});sizes.forEach(function(s){if(o3.indexOf(s)===-1)o3.push(s)});sizes=o3}
...
(async function(){
  // בדיקת טוקן הצגה זמני
  const previewToken = new URLSearchParams(location.search).get('preview');
  if (previewToken) {
    try {
      const r = await fetch('/api/preview-token/' + previewToken);
      if (r.ok) {
        const d = await r.json();
        if (d.valid) {
          const exp = new Date(d.expiresAt);
          const banner = document.createElement('div');
          banner.style.cssText = 'position:fixed;top:0;left:0;right:0;background:linear-gradient(135deg,#c084fc,#818cf8);color:#fff;text-align:center;padding:8px;font-size:13px;font-weight:700;z-index:9999;direction:rtl';
          banner.textContent = `🔗 תצוגה זמנית · תוקף עד ${exp.toLocaleDateString('he-IL')} ${exp.toLocaleTimeString('he-IL')}`;
...
// ערבוב מוצרים לפי חנות — Round Robin
function interleaveByStore(products) {
  ...
  const queues = Object.values(byStore);
  const result = [];
  let i = 0;
...
function selectSort(val, label, el) {
  window._sortVal = val;
...
document.addEventListener('click', e => {
  ...
  <div class="alert-popup-bg" id="alertPopupBg" onclick="closeAlertPopup(event)">
    <div class="alert-popup" onclick="event.stopPropagation()">
      ...
      <div id="alertActiveSummary" style="display:none;background:#fdf2f8;border:1.5px solid #e8c6e0;border-radius:10px;padding:10px 14px;margin-bottom:12px;font-size:13px;color:#7c3f6e;display:flex;align-items:center;justify-content:space-between;gap:8px">
        <span id="alertActiveSummaryText">✅ התראה פעילה</span>
        <div style="display:flex;gap:6px">
          <button onclick="document.getElementById('alertActiveSummary').style.display='none';document.getElementById('alertSizeOption').style.display='';document.getElementById('alertSizeCheck').checked=true;_applyAlertSize(true);if(typeof _currentUpdateBySel==='function')_currentUpdateBySel();" style="background:none;border:1px solid #c48cb3;color:#c48cb3;border-radius:6px;padding:3px 10px;font-size:11px;cursor:pointer">הוסף</button>
          <button onclick="cancelAlert()" style="background:none;border:1px solid #e57373;color:#e57373;border-radius:6px;padding:3px 10px;font-size:11px;cursor:pointer">בטל</button>
        </div>
      </div>
      <div class="alert-option" id="alertPriceOption" onclick="toggleAlertPriceFromDiv(event,this)">
        ...
        <div class="alert-option-label">
          <div>עדכן אותי כשהמחיר יורד</div>
      ...
      <div class="alert-option" id="alertSizeOption" onclick="toggleAlertSizeFromDiv(event,this)">
        ...
        <div class="alert-option-label" style="flex:1">
          <div>עדכן אותי כשמידה נכנסת למלאי</div>
          <div id="alertColorRow" style="display:none;margin-top:6px">
            <div style="font-size:11px;color:#9ca3af;margin-bottom:4px">בחרי צבע:</div>
            <select class="alert-size-select show" id="alertColorSelect" onclick="event.stopPropagation()"></select>
          </div>
      ...
      <div class="alert-popup-actions">
        ...
        <button class="alert-save-btn" id="alertSaveBtn" onclick="saveAlert()">שמור</button>
      </div>
    </div>
  </div>
  ...
  <div class="auth-modal-bg" id="authModalBg" onclick="closeAuthIfBg(event)">
    <div class="auth-modal" onclick="event.stopPropagation()" style="position:relative">
      <button class="auth-modal-close" onclick="closeAuthModal()" aria-label="סגור">✕</button>
      ...
      <div class="auth-modal-body">
        ...
        <!-- Login form -->
        <div id="loginForm">
          <div id="googleLoginBtn" style="display:flex;justify-content:center;margin-bottom:16px;min-height:44px"></div>
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px"><hr style="flex:1;border:none;border-top:1px solid #e5e7eb"><span style="color:#9ca3af;font-size:12px">או עם אימייל</span><hr style="flex:1;border:none;border-top:1px solid #e5e7eb"></div>
        ...
        <div id="registerForm" style="display:none">
          <div id="googleRegisterBtn" style="display:flex;justify-content:center;margin-bottom:16px"></div>
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px"><hr style="flex:1;border:none;border-top:1px solid #e5e7eb"><span style="color:#9ca3af;font-size:12px">או עם אימייל</span><hr style="flex:1;border:none;border-top:1px solid #e5e7eb"></div>
          <div class="auth-field">
        ...
        <div id="profileForm" style="display:none">
          ...
          <div style="margin-top:16px">
            <div style="font-weight:700;font-size:14px;margin-bottom:10px;color:#374151">צפיתי לאחרונה</div>
            <div id="recentlyViewedList" style="display:flex;flex-direction:column;gap:8px;max-height:220px;overflow-y:auto"></div>
          </div>
  ...
  <!-- PROFILE PANEL -->
  <div class="saved-panel-bg" id="profilePanelBg" onclick="if(event.target===this)closeProfilePanel()" style="justify-content:flex-end">
    <div class="saved-panel" onclick="event.stopPropagation()" style="border-radius:0">
      <div class="saved-panel-head">
        <h3>הפרופיל שלי</h3>
        <button class="saved-panel-close" onclick="closeProfilePanel()" aria-label="סגור פרופיל">✕</button>
      </div>
      <div class="saved-panel-body" style="padding:20px">
        <!-- אווטאר ופרטים -->
        <div style="text-align:center;margin-bottom:24px">
          <div style="width:72px;height:72px;border-radius:50%;background:var(--accent);display:flex;align-items:center;justify-content:center;color:#fff;font-size:32px;font-weight:800;margin:0 auto 12px" id="profileAvatar2">?</div>
          <div style="font-weight:700;font-size:17px;margin-bottom:4px" id="profileName2">-</div>
          <div style="color:#9ca3af;font-size:13px" id="profileEmail2">-</div>
          <div style="margin-top:8px" id="profilePlanBadge2"></div>
        </div>
        <!-- כפתורים -->
        <button class="auth-submit" style="margin-bottom:10px" onclick="closeProfilePanel();openSavedPanel()">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:16px;height:16px;vertical-align:middle;margin-left:6px"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
          המוצרים השמורים שלי
        </button>
        <!-- היסטוריית צפייה -->
        <div style="margin-top:20px">
          <div style="font-weight:700;font-size:14px;margin-bottom:10px;color:#374151;border-bottom:1px solid #f3f4f6;padding-bottom:8px">📋 צפיתי לאחרונה</div>
          <div id="recentlyViewedList2" style="display:flex;flex-direction:column;gap:8px;max-height:400px;overflow-y:auto"></div>
        </div>
        <!-- יציאה -->
        <button class="auth-logout-btn" style="margin-top:20px" onclick="doLogout();closeProfilePanel()">יציאה מהחשבון</button>
      </div>
    </div>
  </div>
  ...
  <div class="saved-panel-bg" id="savedPanelBg" onclick="closeSavedIfBg(event)">
    <div class="saved-panel" onclick="event.stopPropagation()">
      <div class="saved-panel-head">
        ...
        <button class="saved-panel-close" onclick="closeSavedPanel()" aria-label="סגור">✕</button>
      </div>
  ...
  function addToRecentlyViewed(product) {
    try {
      let recent = JSON.parse(localStorage.getItem('lookli_recent') || '[]');
      // הסר אם כבר קיים
      recent = recent.filter(p => p.id !== product.id);
      // הוסף בהתחלה
      recent.unshift({ id: product.id, title: product.title, price: product.price, image: product.images?.[0] || product.image_url || '', store: product.store });
      // שמור עד MAX_RECENT
      if (recent.length > MAX_RECENT) recent = recent.slice(0, MAX_RECENT);
      localStorage.setItem('lookli_recent', JSON.stringify(recent));
    } catch(e) {}
  }
  ...
  function renderRecentlyViewed() {
    ...
    if (!el) return;
    try {
      const recent = JSON.parse(localStorage.getItem('lookli_recent') || '[]');
      if (!recent.length) { el.innerHTML = '<div style="color:#9ca3af;font-size:13px">עוד לא צפית במוצרים</div>'; return; }
      el.innerHTML = recent.map(p => `
        <div onclick="closeAuthModal();openProduct(${p.id})" style="display:flex;align-items:center;gap:10px;padding:8px;border-radius:10px;cursor:pointer;border:1px solid #f3f4f6;transition:background .15s" onmouseenter="this.style.background='#fdf2f8'" onmouseleave="this.style.background=''">
          <img src="${escapeHtml(p.image||'')}" style="width:44px;height:44px;object-fit:cover;border-radius:8px;flex-shrink:0" onerror="handleImgError(this)">
          <div style="flex:1;min-width:0">
            <div style="font-size:13px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escapeHtml(p.title||'')}</div>
            <div style="font-size:12px;color:#9ca3af">${escapeHtml(p.store||'')} · ₪${p.price||''}</div>
          </div>
        </div>`).join('');
    } catch(e) {}
  }
  ...
  function openProfilePanel() {
    const el = document.getElementById('profilePanelBg');
    if (!el) return;
    el.classList.add('open');
    if (currentUser) {
      const initials = (currentUser.name || currentUser.email || '?')[0].toUpperCase();
      document.getElementById('profileAvatar2').textContent = initials;
      document.getElementById('profileName2').textContent = currentUser.name || '—';
      document.getElementById('profileEmail2').textContent = currentUser.email || '';
      const badge = document.getElementById('profilePlanBadge2');
      if (badge) badge.innerHTML = currentUser.plan === 'premium'
    ...
    renderRecentlyViewed2();
  }

  function closeProfilePanel() {
    document.getElementById('profilePanelBg')?.classList.remove('open');
  }

  function renderRecentlyViewed2() {
    const el = document.getElementById('recentlyViewedList2');
    if (!el) return;
    try {
      const recent = JSON.parse(localStorage.getItem('lookli_recent') || '[]');
      if (!recent.length) { el.innerHTML = '<div style="color:#9ca3af;font-size:13px;text-align:center;padding:20px">עוד לא צפית במוצרים</div>'; return; }
      el.innerHTML = recent.map(p => `
        <div onclick="closeProfilePanel();openProduct(${p.id})" style="display:flex;align-items:center;gap:10px;padding:8px;border-radius:10px;cursor:pointer;border:1px solid #f3f4f6;transition:background .15s" onmouseenter="this.style.background='#fdf2f8'" onmouseleave="this.style.background=''">
          <img src="${escapeHtml(p.image||'')}" style="width:48px;height:48px;object-fit:cover;border-radius:8px;flex-shrink:0" onerror="handleImgError(this)">
          <div style="flex:1;min-width:0">
            <div style="font-size:13px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escapeHtml(p.title||'')}</div>
            <div style="font-size:12px;color:#9ca3af;margin-top:2px">${escapeHtml(p.store||'')} · ₪${p.price||''}</div>
          </div>
          <svg viewBox="0 0 24 24" fill="none" stroke="#d1d5db" stroke-width="2" style="width:16px;height:16px;flex-shrink:0"><polyline points="15 18 9 12 15 6"/></svg>
        </div>`).join('');
    } catch(e) {}
  }
  ...
  function initGoogleSignIn() {
    ...
    const GOOGLE_CLIENT_ID = '1037279869077-935i5v7lva8q7t0gff3fa4m6rjtf5mn5.apps.googleusercontent.com';
    google.accounts.id.initialize({
      client_id: GOOGLE_CLIENT_ID,
      callback: handleGoogleCredential,
      ux_mode: 'popup',
    });
    const btnConfig = { theme:'outline', size:'large', text:'signin_with', locale:'he', width:280 };
    ...
    if (regEl) google.accounts.id.renderButton(regEl, { ...btnConfig, text:'signup_with' });
  }
  ...
  async function handleGoogleCredential(response) {
    try {
      ...
      const data = await res.json();
      if (!res.ok) return showAuthError(data.error || 'שגיאה בהתחברות עם Google');
      localStorage.setItem('lookli_token', data.token);
  ...
  // אתחל כשה-GSI script נטען
  window.addEventListener('load', () => {
    // דחה אתחולים לא קריטיים לאחר הטעינה
    const idle = window.requestIdleCallback || (cb => setTimeout(cb, 200));
    idle(() => initGoogleSignIn());
  });
  ...
  function checkNetfreeBlock(imgEl) {
    ...
    // תמונות proxy שלנו (/ic?u=) — תמיד אמיתיות, אין צורך לבדוק
    if (imgEl.src.includes('/ic?')) {
      card.dataset.imgOk = '1';
      return;
    }

    // שיטה 1: naturalWidth קטן = חסום
    if (imgEl.naturalWidth > 0 && imgEl.naturalWidth < 20) {
      if (activeNetfree) card.style.display = 'none';
      return;
    }

    // שיטה 2: Performance API — בודק גודל אמיתי שהדפדפן הוריד
    // תמונת נטפרי חסומה = כמה KB בלבד, תמונת מוצר = 30KB+
    try {
      const entries = performance.getEntriesByName(imgEl.src, 'resource');
      if (entries.length > 0) {
        const size = entries[entries.length - 1].encodedBodySize || entries[entries.length - 1].transferSize || 0;
        if (size > 0 && size < 60000) {
          // פחות מ-60KB = תמונת חסימה
          if (activeNetfree) card.style.display = 'none';
          return;
        }
        if (size >= 60000) {
    ...
    // שיטה 3: fallback — canvas לתמונות אחידות (solid color)
    try {
      ...
      // CORS — לא יכולים לבדוק, נניח פתוחה
      card.dataset.imgOk = '1';
    }
  }
  ...
  async function loadSponsored(query) {
    try {
      ...
      if (cards.length > 0) {
        // חשב עמודות לפי רוחב חלון — ללא forced reflow
        const w = window.innerWidth;
        const cols = w >= 1200 ? 4 : w >= 900 ? 3 : w >= 640 ? 2 : 1;
  ...
  async function openAlertPopup(url, title, price, btn) {
    ...
    // איפוס בחירת צבע
    const colorRow = document.getElementById('alertColorRow');
    const colorSel = document.getElementById('alertColorSelect');
    if (colorRow) { colorRow.style.display = 'none'; colorSel.innerHTML = ''; }

    // טען מידות
    const sel = document.getElementById('alertSizeSelect');
    sel.innerHTML = '';
    try {
      const res = await fetch('/api/product-sizes?url=' + encodeURIComponent(url));
      ...
      if (res.ok) {
        ...
        // ── בחירת צבע אם יש כמה צבעים ──
        const colorSizes = p.color_sizes || {};
        // השתמש ב-p.colors לכל הצבעים (כולל אלו שנגמרו מהמלאי)
        const allProductColors = (p.colors || Object.keys(colorSizes)).filter(Boolean);
        const colors = allProductColors.length > 1 ? allProductColors
          : Object.keys(colorSizes).filter(c => colorSizes[c]?.length > 0);
        if (colors.length > 1 && colorRow) {
          colorSel.innerHTML = colors.map(c => `<option value="${c}">${c}</option>`).join('');
          colorRow.style.display = 'none'; // יוצג רק בלחיצה על הצ'קבוקס
          // עדכן מידות לפי צבע ראשון
          const updateBySel = () => {
            const c = colorSel.value;
            sel.disabled = false;
            const rawCs = (colorSizes[c] || []).map(s => s.toUpperCase());
            const isOneSizeColor = rawCs.includes('ONE SIZE');

            // בסיס: all_sizes, או fallback לאיחוד כל color_sizes
            let allBase = (p.all_sizes || []).map(s => s.replace(/^EU_/i,'').trim().toUpperCase()).filter(s => letterSizes.includes(s));
            if (allBase.length === 0) {
              // fallback: כל המידות שמופיעות בכל הצבעים
              const allFromColors = new Set();
              Object.values(colorSizes).forEach(sizes =>
                sizes.forEach(s => { const n=s.toUpperCase(); if(letterSizes.includes(n)) allFromColors.add(n); })
              );
              allBase = [...allFromColors];
            }

            // מה חסר בצבע זה = מה שב-allBase אבל לא ב-color_sizes[צבע]
            const missing = allBase.filter(s => !rawCs.includes(s));

            sel.innerHTML = '';
            if (!isOneSizeColor) {
              if (missing.length > 0) {
                missing.forEach(s => { const o=document.createElement('option'); o.value=s; o.textContent=s; sel.appendChild(o); });
                sel.disabled = false;
                document.getElementById('alertSizeCheck').disabled = false;
              } else {
            ...
            // עדכן show לפי מצב הוי
            _applyAlertSize(document.getElementById('alertSizeCheck').checked);
          };
          colorSel.onchange = updateBySel;
          window._currentUpdateBySel = updateBySel;
          updateBySel();
          // דלג על הלוגיקה הרגילה
          const existing2 = null; // יטופל מתחת
        } else {
          // צבע אחד — קבע אוטומטי
          if (colors.length === 1 && colorRow) colorRow.style.display = 'none';
        }

        // קבע את טווח המוצר: all_sizes אם קיים, אחרת הסקה מהטווח שבמלאי
        let sizeRangeBase;
        if (p.all_sizes && p.all_sizes.length > 0) {
          const norm = p.all_sizes.map(s => s.replace(/^EU_/i,'').trim().toUpperCase());
          sizeRangeBase = norm.filter(s => letterSizes.includes(s));
        } else if (productLetterSizes.length > 0) {
          const idxMin = Math.min(...productLetterSizes.map(s => letterSizes.indexOf(s)));
          const idxMax = Math.max(...productLetterSizes.map(s => letterSizes.indexOf(s)));
          sizeRangeBase = letterSizes.slice(idxMin, idxMax + 1);
        } else {
          sizeRangeBase = [];
        }

        if (sizeRangeBase.length === 0) {
          // אין מידות אות — "כל מידה"
          sel.innerHTML = '<option value="any">כל מידה</option>';
        } else {
          const missingSizes = sizeRangeBase.filter(s => !productLetterSizes.includes(s));
          const hasColorSizes = Object.keys(colorSizes).length > 0;

          if (missingSizes.length === 0 && !hasColorSizes) {
            // כל המידות בטווח המוצר במלאי ואין color_sizes
            sel.innerHTML = '<option value="">כל המידות במלאי ✓</option>';
            sel.disabled = true;
    ...
    try {
      ...
      const summaryDiv = document.getElementById('alertActiveSummary');
      const summaryText = document.getElementById('alertActiveSummaryText');
      if (existing) {
        // הצג סיכום התראה פעילה
        if (existing.alert_size) {
          const colorPart = existing.alert_color ? ` ${existing.alert_color}` : '';
          const sizePart = existing.alert_size === 'any' ? 'כל מידה' : existing.alert_size;
          summaryText.textContent = '✅ פעיל: מידה ' + sizePart + colorPart;
          summaryDiv.style.display = 'flex';
          document.getElementById('alertSizeOption').style.display = 'none';
        } else {
  ...
  function _applyAlertSize(checked) {
    ...
    const hasColors = colorSel && colorSel.options.length > 0;
    ...
    // מסומן — הצג colorRow אם יש צבעים
    if (hasColors && colorRow) colorRow.style.display = 'block';

    // הצג sel רק אם יש אפשרויות ולא disabled
    if (sel.options.length > 0 && !sel.disabled) {
      sel.classList.add('show');
    } else {
      sel.classList.remove('show');
    }
  ...
  function syncAlertSizeCheckbox(chk) {
    _applyAlertSize(chk.checked);
    // אם יש צבעים — עדכן מידות לפי הצבע הנבחר
    if (chk.checked && typeof _currentUpdateBySel === 'function') {
      _currentUpdateBySel();
    }
  ...
  async function saveAlert() {
    ...
    const colorSelEl = document.getElementById('alertColorSelect');
    const alertColor = (colorSelEl && colorSelEl.options.length > 0 && document.getElementById('alertColorRow')?.style.display === 'block')
      ? colorSelEl.value : null;
    ...
    if (!alertPrice && !alertSize) {
      ...
      await fetch('/api/alerts', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + authToken },
        body: JSON.stringify({ product_source_url: _alertUrl, alert_price: alertPrice, alert_size: alertSize, alert_color: alertColor })
      });
...
#cookieBanner p{margin:0;font-size:13px;color:#787878;flex:1;min-width:200px}
#cookieBanner a{color:#e0a1c0;font-weight:600;text-decoration:none}
...
#cookieAccept:hover{background:#c48cb3;transform:translateY(-1px)}
</style>

<div id="cookieBanner">
  <p>האתר משתמש ב-cookies לצורך התחברות ושמירת העדפות, וב-Google Analytics למדידת תנועה — המידע אינו משותף עם צדדים שלישיים לפרסום.</p>
  <button id="cookieAccept" onclick="acceptCookies()">הבנתי ✓</button>
</div>

<script>
(function(){
  if(!localStorage.getItem('cookieConsent')){
    setTimeout(()=>document.getElementById('cookieBanner')?.classList.add('show'), 8000);
  }
})();
