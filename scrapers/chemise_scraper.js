...
// Cloudinary config
cloudinary.config({
  cloud_name: process.env.CLOUDINARY_CLOUD_NAME || 'dghxzffis',
  api_key:    process.env.CLOUDINARY_API_KEY    || '139692763171868',
    ...
    await db.query(
      `INSERT INTO products (store, title, price, original_price, image_url, images, sizes, color, colors, style, fit, category, description, source_url, color_sizes, pattern, fabric, design_details, all_sizes, last_seen, first_seen)
       ...
       ON CONFLICT (source_url) DO UPDATE SET
         ...
         image_url      = CASE WHEN cardinality(EXCLUDED.images) > 0 THEN EXCLUDED.image_url ELSE products.image_url END,
         images         = CASE WHEN cardinality(EXCLUDED.images) > 0 THEN EXCLUDED.images    ELSE products.images    END,
